...
export const STAGES = [
  ...
  { id: 'reference_generation', name: '场景概念图', shortName: '场景概念图' },
...
function ModelSelector({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-72 bg-surface rounded-xl shadow-lg border border-gray-200 p-3 z-50 space-y-2.5">
...
export default function TopBar({
  ...
  return (
    <>
    <header className={clsx("director-topbar fixed top-0 right-0 left-[var(--app-sidebar-width)] z-30 h-14 bg-surface border-b border-gray-200 flex items-center px-4 min-w-0 transition-[left] duration-300", hasSession && "director-workflow-topbar")}>
      {/* Logo & 名称 */}
      <button
        onClick={onHomeClick}
      ...
      >
        ...
        <div className="flex flex-col leading-tight">
          <span className="director-project-title font-bold text-sm text-gray-800 tracking-tight">
            {hasSession ? (projectTitle || '创作项目') : '绘映'}
          </span>
        </div>
      </button>
    ...
    <div className={clsx("flex-shrink-0", hasSession ? "h-28" : "h-14")} />
    </>
  );
}
