      ...
      {output.images && output.images.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {output.images.map((img, i) => (
            <a key={i} href={toMediaUrl(img)} target="_blank" rel="noopener noreferrer" className="group block rounded-xl border border-gray-200 bg-surface overflow-hidden">
              <img src={toMediaUrl(img)} alt={`output-${i}`} className="w-full h-56 object-contain bg-gray-50" />
              <div className="px-3 py-2 text-xs text-gray-500 group-hover:text-blue-600 border-t border-gray-100">查看图片</div>
            </a>
          ))}
        </div>
      )}
      {output.video_path && (
        <div className="rounded-xl border border-gray-200 bg-surface overflow-hidden">
          ...
          <div className="px-3 py-2 border-t border-gray-100">
            <a href={toMediaUrl(output.video_path)} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
              查看视频
            </a>
          </div>
        </div>
    ...
    <div className="mb-4">
      ...
      <div className="flex gap-2 mb-2">
        ...
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${
            inputMode === 'file' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-100'
          }`}
        >
          本地上传
        </button>
          ...
          {value && !isUrl ? (
            ...
            >
              ...
              <p className="text-sm text-gray-500">
                拖拽图片到此处，或 <span className="text-blue-600">点击选择文件</span>
              </p>
    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        <>
                ...
                <button
                  ...
                  className={`p-4 rounded-xl border-2 transition-all text-center ${
                    activeTool === tool.id
                      ...
                      : 'border-gray-200 bg-surface text-gray-600 hover:border-gray-300 hover:shadow-sm'
                  }`}
                >
            ...
            {/* 输入区域 */}
            <div className="bg-surface rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
              {/* 模型选择 */}
              <div className="mb-4">
              ...
              <button
                ...
                className="w-full py-3 px-6 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
            ...
            {(currentOutput || error) && (
              <div className={`rounded-2xl border p-6 ${error ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {!error && currentOutput && (
                    <button onClick={copyResult} className="p-2 rounded-lg hover:bg-surface/50 transition-colors" title="复制结果">
          ...
          <section className="mt-10">
            ...
            {history.filter(record => record.tool === activeTool).length === 0 ? (
              <div className="h-32 rounded-xl border border-dashed border-gray-200 bg-surface/70 flex items-center justify-center text-sm text-gray-400">
                暂无历史记录
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {history.filter(record => record.tool === activeTool).map(record => (
                  <div
                    ...
                    className={`bg-surface rounded-xl border border-gray-200 p-4 hover:border-blue-300 hover:shadow-sm transition-all ${manageMode ? '' : 'cursor-pointer'}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-2">
                          <span className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">{record.model}</span>
