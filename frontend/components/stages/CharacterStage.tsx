...
function AssetRow({
  ...
  return (
    <div className={`flex flex-col xl:flex-row border rounded-xl overflow-hidden bg-surface ${
          ...
          {!isStageRunning && (
            isEditing ? (
              <div className="ml-auto flex gap-1">
                ...
                <button
                  onClick={onSaveEdit}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium text-white bg-blue-500 hover:bg-blue-600"
                >
...
export default function CharacterStage({ state, sessionId, onConfirm, onIntervene, onRegenerate, onUpdateArtifact, onSaveSelections, showConfirm, isRunning, hasPendingItems, hasNextStageStarted }: StageViewProps) {
  ...
  const pendingRegenerations = useRef(new Set<string>());
  ...
  const handleRegenerate = async (type: 'characters' | 'settings', id: string) => {
    if (pendingRegenerations.current.has(id)) return;
    pendingRegenerations.current.add(id);
    setRegeneratingIds(prev => new Set(prev).add(id));
    try {
      await onIntervene(type === 'characters'
        ? { regenerate_characters: [id] }
        : { regenerate_settings: [id] });
    } finally {
      pendingRegenerations.current.delete(id);
