...
function ImageGallery({
  ...
  return (
    <div className="director-gallery relative group">
      <button className="director-image-preview" onClick={() => setLightboxIndex(Math.max(0, versions.indexOf(selected)))} aria-label="放大当前场景概念图">
        <img src={assetUrl(versions.includes(selected) ? selected : versions[0])} alt="当前选中的场景概念图" />
      ...
      {versions.length > 1 && (
        <>
          <button
            onClick={() => scroll('left')}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-7 h-7 rounded-full bg-surface/90 shadow border border-gray-200 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
          >
            <ChevronLeft className="w-4 h-4 text-gray-600" />
          </button>
          <button
            onClick={() => scroll('right')}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-7 h-7 rounded-full bg-surface/90 shadow border border-gray-200 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
          >
          ...
          return (
            <div
              ...
              className={`flex-shrink-0 cursor-pointer rounded-lg overflow-hidden transition-all ${
                isSelected
                  ? 'ring-3 ring-blue-500 shadow-lg shadow-blue-200'
            ...
            >
              ...
              <div className={`text-center text-[10px] py-0.5 ${
                isSelected ? 'bg-blue-500 text-white font-medium' : 'bg-gray-50 text-gray-400'
              }`}>
...
function SceneRow({
  ...
  return (
    <div className={`director-scene-row flex flex-col xl:flex-row border rounded-xl overflow-hidden bg-surface ${
      ...
      <div className="w-full xl:w-80 xl:flex-shrink-0 p-4 border-b xl:border-b-0 xl:border-r border-gray-100 flex flex-col">
        <div className="flex items-center gap-2 mb-2">
          <span className="flex items-center justify-center h-6 px-1.5 rounded-full bg-blue-100 text-blue-700 text-[10px] font-bold flex-shrink-0 whitespace-nowrap">
        ...
        {isEditing ? (
          <textarea
            ...
            rows={5}
            className="h-[120px] text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg p-2 resize-none focus:outline-none focus:ring-1 focus:ring-blue-300"
          />
        ) : (
                ...
                <>
                  ...
                  {canGenerateMissing && (
                    <button
                      onClick={onRegenerate}
                      disabled={isRegenerating}
                      className="mt-1 inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors disabled:text-gray-400 disabled:bg-gray-100 disabled:cursor-not-allowed"
                    >
                      <RefreshCw className="w-2.5 h-2.5" />
                      生成
                    </button>
...
export default function ReferenceStage({ state, sessionId, onConfirm, onIntervene, onRegenerate, onUpdateArtifact, onSaveSelections, showConfirm, isRunning, hasPendingItems, hasNextStageStarted, scriptArtifact }: StageViewProps) {
  ...
  const focusedScene = scenes.find(scene => scene.id === focusedSceneId) || scenes[0];
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 min-w-0 overflow-y-auto p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-lg font-semibold text-gray-800">场景概念图生成</h2>
        </div>
        <p className="text-sm text-gray-500 mb-6">
          基于角色/场景素材 + 分镜视觉描述，使用图生图生成场景概念图
        </p>

        {/* 运行中 */}
        {state.status === 'running' && (
          <StageProgress message={state.progressMessage} fallback="正在生成场景概念图..." progress={state.progress} color="emerald" />
        )}
        ...
        {hasScenes && focusedScene && (
          <div className="director-reference-workspace">
            <aside className="director-shot-list" aria-label="分镜列表">
              <h3>分镜列表 <span>{scenes.length} 个镜头</span></h3>
              {scenes.map((scene, index) => <button key={scene.id} onClick={() => setFocusedSceneId(scene.id)} className={scene.id === focusedScene.id ? 'is-current' : ''} aria-pressed={scene.id === focusedScene.id}>
                {getSelected(scene) || scene.versions[0] ? <img src={assetUrl(getSelected(scene) || scene.versions[0])} alt={scene.name || '场景概念图'} /> : <div className="director-shot-placeholder"><ImageIcon size={24} /><span>{scene.status === 'failed' ? '生成失败' : scene.status === 'running' ? '生成中' : '等待场景概念图'}</span></div>}
                <span className="block px-2 pt-2 text-[10px] text-gray-400">第 {scene.episode || 1} 集{episodeTitleMap[scene.episode || 1] ? ` · ${episodeTitleMap[scene.episode || 1]}` : scene.episode_title ? ` · ${scene.episode_title}` : ''}</span>
                <span className="director-shot-name"><b>{String(index + 1).padStart(2, '0')}</b>{scene.name || scene.id}</span>
              </button>)}
            </aside>
            <SceneRow
              key={focusedScene.id}
              scene={focusedScene}
              editDesc={editDescs[focusedScene.id] ?? focusedScene.description}
              onDescChange={val => setEditDescs(prev => ({ ...prev, [focusedScene.id]: val }))}
              onSavePrompt={() => handleSavePrompt(focusedScene.id)}
              onRegenerate={() => handleRegenerate(focusedScene.id)}
              onSelectVersion={path => handleSelectVersion(focusedScene.id, path)}
              isStageRunning={state.status === 'running'}
              isRegenerating={regeneratingIds.has(focusedScene.id)}
              isEditing={editingIds.has(focusedScene.id)}
              onToggleEdit={() => handleToggleEdit(focusedScene.id)}
              onCancelEdit={() => handleCancelEdit(focusedScene.id)}
              canEdit={canEdit}
              getSelected={getSelected}
              allowMissingGenerate={state.status !== 'pending'}
              onUploadImage={file => handleUploadImage(focusedScene.id, file)}
              isUploading={uploadingIds.has(focusedScene.id)}
            />
          </div>
        )}
      ...
      {!isRunning && hasScenes && <p className="px-6 py-2 text-sm text-gray-500">下一步：检查并选定场景概念图，确认后生成 {scenes.length} 个视频片段。</p>}
      {/* 底部操作栏 */}
      <StageActions
        status={state.status}
        onConfirm={onConfirm}
        confirmLabel={"确认场景概念图，生成视频（"+scenes.length+"）"}
        confirmDisabled={!hasScenes || scenes.some(scene=>!(selectedVersions[scene.id] || scene.selected))}
