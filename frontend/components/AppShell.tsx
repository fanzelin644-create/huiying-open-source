...
import { CheckCircle2, ChevronLeft, ChevronRight, Clapperboard, Clock, Hexagon, Home, LayoutDashboard, Loader2, PanelLeftOpen, Repeat2, Settings, Trash2, UserRound } from 'lucide-react';
...
const NAV_ITEMS = [
  { href: '/', label: '创作工作台', icon: Home },
  { href: '/canvas', label: '无限画布', icon: LayoutDashboard },
...
const COMPLETED_TASK_DISMISS_KEY = 'huiying.dismissed-completed-tasks';
const SIDEBAR_OPEN_KEY = 'huiying.sidebar-open';
...
function loadSidebarOpen(): boolean {
  ...
  return stored === null ? window.matchMedia('(min-width: 1024px)').matches : stored === 'true';
}
...
function TaskPanel({
  ...
  return (
    <section className="director-task-panel h-[16vh] min-h-24 border-t border-gray-100 p-3">
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    active ? 'border-blue-100 bg-blue-50/60' : 'border-gray-100 bg-surface hover:border-blue-200 hover:bg-blue-50/40'
                  )}
                >
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setOpen(mobile.matches ? false : loadSidebarOpen());
    const onResize = () => setOpen(mobile.matches ? false : loadSidebarOpen());
  ...
  return (
    <div
      className="director-shell min-h-screen bg-gray-50 text-gray-800"
      style={{ '--app-sidebar-width': open ? '15rem' : '0px' } as CSSProperties}
    >
      {open && <button className="director-sidebar-backdrop" aria-label="关闭导航" onClick={() => setSidebarOpen(false)} />}
      <aside
        className={clsx(
          'director-sidebar fixed inset-y-0 left-0 z-40 border-r border-gray-200 bg-surface shadow-sm transition-all duration-300',
        ...
        <div className={clsx('flex h-full flex-col overflow-hidden transition-opacity duration-200', open ? 'opacity-100' : 'opacity-0')}>
          <div className="flex h-16 items-center px-4 border-b border-gray-100">
            <div className="flex items-center gap-2 min-w-0">
              <BrandMark />
              <div><span className="text-lg font-semibold text-gray-800 tracking-wider">绘映</span><p className="director-brand-caption">AI FILM STUDIO</p></div>
            </div>
          </div>
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  onClick={() => { if (window.matchMedia("(max-width: 760px)").matches) setOpen(false); }}
                  className={clsx(
                    'flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors',
                    active
                      ? 'director-nav-active bg-blue-50 text-blue-600'
          ...
          <div className="relative border-t border-gray-100 p-3">
            {(() => {
              ...
              return (
                <>
                  {settingsMenuOpen && (
                    <div className="absolute bottom-[62px] left-3 right-3 rounded-xl border border-gray-200 bg-surface p-2 shadow-lg">
                      <button
                        type="button"
                        onClick={() => {
                          setSettingsMenuOpen(false);
      ...
      {open && (
        <button
          ...
          className="fixed left-60 top-1/2 z-50 h-14 w-7 -translate-y-1/2 rounded-r-xl border border-l-0 border-gray-200 bg-surface text-gray-400 shadow-sm hover:w-9 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 flex items-center justify-center transition-all"
          title="收起侧边栏"
        >
      ...
      {!open && (
        <button
          ...
          className="fixed left-0 top-1/2 z-50 h-14 w-7 -translate-y-1/2 rounded-r-xl border border-l-0 border-gray-200 bg-surface text-gray-400 shadow-sm hover:w-9 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 flex items-center justify-center transition-all"
          title="打开侧边栏"
        >
      ...
      <main className={clsx('director-main min-h-screen min-w-0 overflow-x-hidden transition-[margin] duration-300', open ? 'ml-60' : 'ml-0')}>
        {children}
      </main>
    </div>
  );
