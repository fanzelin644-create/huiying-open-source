...
export default function WorkflowPanel() {
    ...
    try {
      ...
      for await (const event of parseStreamEvents(response)) {
        if (event.type === 'progress') {
          ...
          if (stageId === 'storyboard') throw new Error(event.content || '分镜保存失败');
        }
      }
    } catch (error: any) {
      console.error('Intervention error:', error);
      if (stageId === 'storyboard') throw error;
    } finally {
      if (!isBackgroundItemRegeneration) {
        setIsRunning(false);
      }
  ...
  const artifactHasFailure = (value: unknown): boolean => {
    if (!value || typeof value !== 'object') return false;
    if (Array.isArray(value)) return value.some(artifactHasFailure);
    const record = value as Record<string, unknown>;
    return record.status === 'failed' || Object.values(record).some(artifactHasFailure);
  };
  ...
  for (let i = 0; i < STAGE_ORDER.length; i++) {
    ...
    if (stageStates[s]?.status !== 'running' && artifactHasFailure(stageStates[s]?.artifact)) {
  ...
  const hasError = Object.values(stageStates).some(s => s.status === 'error' || artifactHasFailure(s.artifact));
  const allCompleted = Object.values(stageStates).every(s => s.status === 'completed');
  ...
  } else if (Object.values(stageStates).some(s => s.status === 'completed')) {
  ...
  const renderStageContent = () => {
    ...
    const hasSubsequentExecution = STAGE_ORDER.slice(idx + 1).some(
      s => {
        const next = stageStates[s];
        if (!next) return false;
        if (['running','completed','error','stopped'].includes(next.status)) return true;
        const items = next.artifact?.scenes || next.artifact?.clips || [];
        return next.status === 'waiting' && items.some((item: any)=>item.selected || item.versions?.length);
      }
    );
  ...
  return (
    <div className="flex h-screen w-full min-w-0 flex-col bg-gray-50/50">
      <TopBar
        projectTitle={projectParams?.idea}
