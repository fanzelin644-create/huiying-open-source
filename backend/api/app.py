...
async def lifespan(app: FastAPI):
    logger.info("Starting HuiYing API")
    logger.info("Code directory mounted at /code: %s", settings.CODE_DIR)
    yield
    logger.info("HuiYing API shutdown complete")


app = FastAPI(title="绘映", version="2.0.0", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "绘映", "version": "2.0.0", "health": "/api/health"}
