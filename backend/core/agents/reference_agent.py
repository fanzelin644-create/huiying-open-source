    ...
    def _generate_one(self, img_client, sid: str, segment: dict,
        ...
        for version in range(max_versions):
            ...
            try:
                ...
                if eval_result.get("evaluation_status") == "unavailable":
                    eval_result["final_visual_prompt"] = current_visual_prompt
                    self._report_progress("参考图", f"图片已生成，自动评估不可用，请人工检查：{segment_id}", 0)
                    ...
                    with ThreadPoolExecutor(max_workers=concurrency) as executor:
                        ...
                        for fut in as_completed(futs):
                            ...
                            if result_path:
                                ...
                                self._report_progress("参考图", (f"图片已生成，自动评估不可用，请人工检查：{segment_id_done}" if isinstance(eval_result, dict) and eval_result.get("evaluation_status") == "unavailable" else f"完成: {segment_id_done}"), pct, data={
                                    "asset_complete": {
