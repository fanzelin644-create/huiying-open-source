...
class WorkflowEngine:
    ...
    def _sync_artifacts_cross_stages(self, state: WorkflowState, stage: WorkflowStage, payload: Dict):
        ...
        if stage == WorkflowStage.STORYBOARD:
            ...
            from core.storyboard_scope import scoped_episodes
            episodes = scoped_episodes(episodes)
            all_sync_clips = []
            for ep in episodes:
                ...
                for s_i, seg in enumerate(ep.get("segments", []), 1):
                    ...
                    all_sync_clips.append({
                        ...
                        "episode": ep_n,
                        "index": s_i,
                        "name": f"第{ep_n}集-片段{seg.get('segment_number', s_i)}"
                    })
            ...
            allowed_ids = {item["segment_id"] for item in all_sync_clips}
            all_scenes = {item["id"]: item for item in ref_art.get("excluded_scenes", []) + ref_art.get("scenes", [])}
            ref_art["excluded_scenes"] = [item for key, item in all_scenes.items() if key not in allowed_ids]
            existing_scenes = [item for key, item in all_scenes.items() if key in allowed_ids]
            ...
            all_clips = {item["id"]: item for item in video_art.get("excluded_clips", []) + video_art.get("clips", [])}
            video_art["excluded_clips"] = [item for key, item in all_clips.items() if key not in allowed_ids]
            existing_clips = [item for key, item in all_clips.items() if key in allowed_ids]
            for c_info in all_sync_clips:
                id = c_info["segment_id"]
    ...
    async def execute_stage(self,
        ...
        with self._state_lock:
            ...
            if stage in (WorkflowStage.REFERENCE_GENERATION, WorkflowStage.VIDEO_GENERATION, WorkflowStage.POST_PRODUCTION):
                from core.storyboard_scope import scope_artifacts
                input_data["_session_artifacts"] = scope_artifacts(state.artifacts)
